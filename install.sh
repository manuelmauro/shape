...
# shellcheck shell=sh
#
# shape install script
# Run with: curl -sSfL https://raw.githubusercontent.com/manuelmauro/shape/main/install.sh | sh
#
# This script installs shape to ~/.cargo/bin (if cargo is available) or ~/.local/bin
# It will attempt to download a pre-built binary first, falling back to cargo install

set -e

REPO="manuelmauro/shape"
BINARY_NAME="shape"
...
install_binary() {
    ...
    # Find the binary (handles both shape and shape.exe)
...
main() {
    printf "\n"
    printf "${BOLD}shape installer${NC}\n"
    printf "===============\n\n"
